...
test("MessageList shows empty state when no messages", () => {
  ...
  // Empty state is handled by ChatInterface; MessageList renders no message rows
  const messageRows = container.querySelectorAll(".rounded-xl");
...
test("MessageList renders messages with parts", () => {
  ...
  // ToolInvocationBadge renders a human-readable label, not the raw tool name
